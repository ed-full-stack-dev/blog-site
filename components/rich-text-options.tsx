...
import { CodeBlock, getLanguageFromCode } from "../utils/code-blocks";
...
export const RICHTEXT_OPTIONS = {
  renderNode: {
    ...
    [BLOCKS.OL_LIST]: (node: Node, children: ReactNode) => {
      return <ol className="list-decimal pl-2 md:pl-3 text-xl ">{children}</ol>;
    },
    [BLOCKS.UL_LIST]: (node: Node, children: ReactNode) => {
      return <ul className="list-disc  md:pl-3 ">{children}</ul>;
    },
    [BLOCKS.LIST_ITEM]: (node: Node, children: ReactNode) => {
      return <li className="ml-2 mb-2 text-gray-700">{children}</li>;
    },
